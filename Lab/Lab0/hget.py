...
def connect_to_server(server_name):
    ...
    """ 
    Cuando le pasas un hostname (nombre de dominio, como "www.google.com") 
    la función consulta el Sistema de Nombres de Dominio (DNS) para obtener la dirección IP asociada a ese nombre.
    """
    ...
    """
    Primero creamos un socket de tipo TCP
    donde el address family es AF_INET (pueden ser IPv4, IPv6, etc), AF_INET le dice a python que trabajara con IPv4
    y el tipo es SOCK_STREAM (TCP, UDP), SOCK_STREAM trabaja con TCP

    Luego nos conectamos al servidor en la direccion IP obtenida y el puerto HTTP (80)
    (Tener en cuenta que s.connect espera una tupla por lo que hay que poner otro ())
    """
...
"""
Los nombres de dominio generalmente usan caracteres ASCII (A-Z, 0-9 y guion "-").
Con el mecanismo **Punycode** es posible escribir paginas con caracteres especiales, por ejemplo:
...
El término **Encoding** se refiere a la forma en que la información se representa en un formato específico. En este caso, Punycode es un tipo de encoding
que permite que los nombres de dominio internacionales (IDN, *Internationalized Domain Names*) funcionen en Internet sin problemas.
"""